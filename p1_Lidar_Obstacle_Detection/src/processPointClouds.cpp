...
typename pcl::PointCloud<PointT>::Ptr ProcessPointClouds<PointT>::FilterCloud(typename pcl::PointCloud<PointT>::Ptr cloud, float filterRes, Eigen::Vector4f minPoint, Eigen::Vector4f maxPoint)
{
    ...
    // Convert the points to voxel grid
    pcl::VoxelGrid<PointT> vx_g;
    vx_g.setInputCloud (cloud);
    vx_g.setLeafSize (filterRes, filterRes, filterRes);
    vx_g.filter (*cloud_filtered);
    std::cerr << "Voxeled " << cloud_filtered->points.size () << std::endl;

    // Crop the scene
    pcl::CropBox<PointT> reg;
    reg.setMin(minPoint);
    reg.setMax(maxPoint);
    reg.setInputCloud(cloud_filtered);
    reg.filter(*cloud_filtered);
    std::cerr << "reg " << cloud_filtered->points.size () << std::endl;
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
    ...
    pcl::EuclideanClusterExtraction<PointT> euc_c;
    euc_c.setClusterTolerance (clusterTolerance);
    euc_c.setMinClusterSize (minSize);
    euc_c.setMaxClusterSize (maxSize);
    euc_c.setSearchMethod (tree);
    euc_c.setInputCloud (cloud);
    euc_c.extract (cluster_indices);
